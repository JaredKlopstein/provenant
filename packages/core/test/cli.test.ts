...
async function cli(args: string): Promise<{ code: number; out: string; err: string }> {
  ...
  const code = await runCli([...argv, '--store', store], io);
  return { code, out, err };
}
...
describe('the cold-agent path', () => {
  it('gets from nothing to a verified receipt using only the quickstart', async () => {
    const manifest = JSON.parse((await cli('discover')).out) as {
      quickstart: Array<{ action: string; arguments: Record<string, unknown> }>;
    };

    // Step 1: init
    const init = await json('init --display-name cold-agent');
    ...
    expect((await json('chain head')).body.result).toMatchObject({ count: 0 });
    ...
    // Step 4: verify
    const verify = await json('chain verify');
  ...
  it('every successful response carries ready-to-execute next_actions', async () => {
    await json('init');
    const rec = await json(`record --action email.send`);
...
describe('CLI conventions', () => {
  beforeEach(async () => { await json('init'); });

  it('supports both dotted and spaced command forms', async () => {
    expect((await json('chain.verify')).code).toBe(0);
    expect((await json('chain verify')).code).toBe(0);
  });

  it('emits JSON on stdout for success and stderr for errors', async () => {
    const ok = await cli('chain head --json');
    expect(ok.out.trim().startsWith('{')).toBe(true);
    expect(ok.err).toBe('');

    const bad = await cli('record --json');
    expect(bad.err.trim().startsWith('{')).toBe(true);
    expect(bad.code).toBe(1);
  });

  it('parses JSON object flags and comma-separated array flags', async () => {
    const r = await json(`record --action t --action-detail '{"a":{"b":[1,2]}}'`);
    const receipt = (r.body.result as { receipt: { action_detail: unknown } }).receipt;
    expect(receipt.action_detail).toEqual({ a: { b: [1, 2] } });

    const q = await json('receipts query --fields seq,action --limit 5');
    const items = (q.body.result as { items: Array<Record<string, unknown>> }).items;
    expect(Object.keys(items[0]!)).toEqual(['seq', 'action']);
  });

  it('refuses --dry-run on read actions with an explanatory error', async () => {
    const r = await json('chain verify --dry-run');
    expect(r.code).toBe(1);
    expect((r.body.error as { message: string }).message).toMatch(/read action/);
  });

  it('paginates with opaque cursors and an explicit truncated flag', async () => {
    for (let i = 0; i < 7; i++) await json(`record --action bulk.op --action-detail '{"i":${i}}'`);

    const first = await json('receipts query --limit 3 --fields seq');
    ...
    const second = await json(`receipts query --limit 3 --fields seq --cursor ${p1.next_cursor}`);
  ...
  it('rejects a malformed cursor with a recovery path', async () => {
    const r = await json('receipts query --cursor not-a-cursor');
    expect(r.code).toBe(1);
  ...
  it('never emits an untyped error', async () => {
